...
class CSIModelConfig:
    ...
    def build_model(self, n_unit_lstm=200, n_unit_atten=400, feature=90):
        """
        Returns the Tensorflow Model which uses AttenLayer
        """
        if self._downsample > 1:
            length = len(np.ones((self._win_len,))[::self._downsample])
            x_in = tf.keras.Input(shape=(length, feature))
        else:
            x_in = tf.keras.Input(shape=(self._win_len, feature))
    ...
    def build_ourmodel(self, label=4, n_unit_lstm=200, n_unit_atten=400, feature=90):
        """
        Returns the Tensorflow Model which uses AttenLayer
        """
        if self._downsample > 1:
        ...
        x_tensor = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(units=n_unit_lstm, return_sequences=True))(x_in)
        x_tensor = AttenLayer(n_unit_atten)(x_tensor)
        pred = tf.keras.layers.Dense(label, activation='softmax')(x_tensor)
        model = tf.keras.Model(inputs=x_in, outputs=pred)
        return model
...
if __name__ == "__main__":
    ...
    numpy_tuple = cfg.load_csi_data_from_files(
        ('X_bed_win_1000_thrshd_60percent_step_200.npz', 
        'X_fall_win_1000_thrshd_60percent_step_200.npz',
        'X_pickup_win_1000_thrshd_60percent_step_200.npz',
        'X_run_win_1000_thrshd_60percent_step_200.npz',
        'X_sitdown_win_1000_thrshd_60percent_step_200.npz',
        'X_standup_win_1000_thrshd_60percent_step_200.npz',
        'X_walk_win_1000_thrshd_60percent_step_200.npz'))
